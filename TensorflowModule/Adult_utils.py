...
def create_alphas(dim) :
    alphas = {
        ...
        'uniform'                 : [1          for i in range(dim)], #Eqivalent to a uniform sample(random_state = 0)
        ...
        'extremely heterogeneous' : [1/1000     for i in range(dim)] #Unusable -> Too low values (2^-128=> np.NaN)
    }
    return alphas
...
def save_model(model, name, iteration) :
    loc = '../SharedFiles/Client_Models/iteration='+str(iteration)+'/Client'+str(name)+'/' # save location
    print(f'saving model at location :  {loc}')
    try :
        os.makedirs(os.path.dirname(loc), exist_ok=True)
    except OSError:
        print (f"[save_model-error] Error while opening gilr {loc}")
        sys.exit()
...
def save_metric(metric_value, name, iteration):
    try:
        ...
        loc = f'../SharedFiles/Fairness_Metrics/iteration={iteration}/Client{name}.txt'
        ...
        with open(loc, "w+") as f:
            f.write(str(metric_value))  # Write the metric value to the file

    except OSError:
        print(f"[save_metric-error] Error while opening or creating file {loc}")
        sys.exit()
